...
: INTERPRET
	begin
		word ( -- str ) find ( cfa true | str false )
		if ( cfa )
			execute		\ call to addr from stack
		else ( str )
			abort		\ dump state, stacks and restart
		endif
	again
;
\end{lstlisting}
...
\clearpage
\noindent
if/else is complex to understand: it uses forward jumps and \term{backpatching}
with \emph{one pass compiling}:
\begin{description}
	\item[if] compiles \verb|?jmp| conditional jump command
	\item[else]
	\item[endif]
\end{description}
