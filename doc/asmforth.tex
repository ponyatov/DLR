...
using nested \term{call}. Every time when call occurs, current execution
...
\term{return address} from stack, and return execution on next command after
used \verb|call| command\note{there is some variant of pure threaded code you
can see in \cite{thbell}\ --- only addresses without call opcode, and special
address for ret}.
